...
console.log("[1.1I Auto-Loop] Starting Revenue Outcome Intelligence auto-loop...");

for (let i = 1; i <= REQUIRED; i++) {
  console.log(`\n[Auto-Loop] Attempt ${i} (dry_run)...`);
  try {
    execSync("node scripts/ai-company-revenue-outcome-intelligence.mjs --mode dry_run --timezone America/New_York", { cwd: ROOT, stdio: "inherit" });
  ...
  try {
    execSync("node scripts/ai-company-revenue-outcome-intelligence.mjs --mode sandbox --timezone America/New_York --signal=rec_pilot_002:replied", {
      cwd: ROOT,
      stdio: "inherit"
    });
    execSync("node packages/db/src/_verify-1.1i.mjs", { cwd: ROOT, stdio: "inherit" });
