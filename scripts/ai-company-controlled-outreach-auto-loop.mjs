#!/usr/bin/env node
/**
 * Milestone 1.0X: Controlled Consented Outreach Pilot Auto-Loop
...
async function runMission() {
  try {
    execSync("node scripts/ai-company-run-controlled-outreach-pilot-mission.mjs", {
      cwd: ROOT,
      stdio: "inherit",
      encoding: "utf8",
    });
...
async function main() {
  ensureDirs();
  console.log("[1.0X Auto-Loop] Starting pilot outreach auto-loop...");
  ...
  const report = {
    ...
    verdict: consecutivePasses >= REQUIRED_STABLE_PASSES
      ? "CONTROLLED_OUTREACH_AUTO_LOOP_STABLE"
      : "CONTROLLED_OUTREACH_AUTO_LOOP_UNSTABLE",
  };
  const reportPath = path.join(LOG_DIR, "controlled-outreach-auto-loop-report.json");
