...
console.log("[Premerge Simulate 1.1G] Starting pre-merge simulation...");
...
const tracked = execSync("git ls-files artifacts/ai-company/mission-1.1g/generated/", { cwd: ROOT }).toString().trim().split("\n").filter(Boolean);
const runtime = ["followup-trigger-plan.json", "followup-trigger-scorecard.json"];
...
console.log("[Premerge Simulate] Checking lib/followup/ for secrets...");
const SCRIPTS = [
...
console.log("[Premerge Simulate] Final Verdict: FOLLOWUP_PREMERGE_PASS");
