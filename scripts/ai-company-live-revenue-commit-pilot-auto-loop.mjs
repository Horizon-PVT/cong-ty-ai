...
console.log("[1.1K Auto-Loop] Starting Live Revenue Commit Pilot auto-loop...");

for (let i = 1; i <= REQUIRED; i++) {
  console.log(`\n[Auto-Loop] Attempt ${i} (dry_run)...`);
  try {
    execSync("node scripts/ai-company-live-revenue-commit-pilot.mjs --mode dry_run --timezone America/New_York", { cwd: ROOT, stdio: "inherit" });
  ...
  try {
    execSync("node scripts/ai-company-live-revenue-commit-pilot.mjs --mode sandbox --timezone America/New_York", {
      cwd: ROOT,
      stdio: "inherit",
