#!/usr/bin/env node
/**
 * Milestone 1.0U: Email Live Pilot Harness Pre-Merge Simulation
...
async function main() {
  ensureDirs();
  console.log("[Premerge Simulate] Starting 1.0U pre-merge simulation...");

  // 1. Run verifier
  console.log("[Premerge Simulate] Running 1.0U verifier...");
  try {
  ...
  const runnerPath = path.join(ROOT, "scripts", "ai-company-run-email-live-pilot-mission.mjs");
  const runnerSrc = fs.readFileSync(runnerPath, "utf8");
  const forbiddenArtifactNames = ["daily-email-sandbox-payload", "daily-live-action-gateway", "daily-email-live-readiness-payload"];
  let metaSafe = true;
  ...
  const report = {
    ...
    verdict: "EMAIL_LIVE_PILOT_PREMERGE_PASS",
  };
  const reportPath = path.join(LOG_DIR, "email-live-pilot-premerge-simulate-report.json");
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  console.log("[Premerge Simulate] Wrote logs/email-live-pilot-premerge-simulate-report.json");
  console.log("[Premerge Simulate] Final Verdict: EMAIL_LIVE_PILOT_PREMERGE_PASS");
}
