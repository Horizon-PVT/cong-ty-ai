...
console.log("[Premerge Simulate 1.1C] Starting pre-merge simulation...");
...
const tracked = execSync("git ls-files artifacts/ai-company/mission-1.1c/generated/", { cwd: ROOT }).toString().trim().split("\n").filter(Boolean);
const runtime = ["meeting-scheduling-ledger.json", "revenue-conversation-scorecard.json"];
...
console.log("[Premerge Simulate] Checking lib/meeting/ and lib/revenue-conversation/ for secrets...");
const SCRIPTS = [
...
const FORBIDDEN = [/pat-[A-Za-z0-9\-]{10,}/, /re_[A-Za-z0-9]{20,}/, /sk-[A-Za-z0-9]{20,}/, /CALENDAR_API_KEY\s*=\s*\S+/];
