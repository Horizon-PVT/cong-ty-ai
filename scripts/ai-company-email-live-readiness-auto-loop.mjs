#!/usr/bin/env node
/**
 * Milestone 1.0T: Email Live Readiness Auto-Loop
...
async function runMission() {
  try {
    execSync("node scripts/ai-company-run-email-live-readiness-mission.mjs", {
      cwd: ROOT,
      stdio: "inherit",
      encoding: "utf8",
    });
...
async function main() {
  ensureDirs();
  console.log("[1.0T Auto-Loop] Starting Email Live Readiness auto-loop...");
  console.log(`[1.0T Auto-Loop] Max iterations: ${MAX_ITERATIONS}, Required stable passes: ${REQUIRED_STABLE_PASSES}`);
  ...
  const report = {
    ...
    verdict: consecutivePasses >= REQUIRED_STABLE_PASSES
      ? "EMAIL_LIVE_READINESS_AUTO_LOOP_STABLE"
      : "EMAIL_LIVE_READINESS_AUTO_LOOP_UNSTABLE",
  };
  const reportPath = path.join(LOG_DIR, "email-live-readiness-auto-loop-report.json");
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  console.log(`[Auto-Loop] Wrote logs/email-live-readiness-auto-loop-report.json`);
