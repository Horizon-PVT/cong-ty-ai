...
console.log("[1.1M Auto-Loop] Starting Revenue Collection & Kickoff auto-loop...");

for (let i = 1; i <= REQUIRED; i++) {
  console.log(`\n[Auto-Loop] Attempt ${i} (dry_run)...`);
  try {
    execSync("node scripts/ai-company-revenue-collection-kickoff.mjs --mode dry_run --timezone America/New_York", { cwd: ROOT, stdio: "inherit" });
  ...
  try {
    execSync("node scripts/ai-company-revenue-collection-kickoff.mjs --mode sandbox --timezone America/New_York", {
      cwd: ROOT,
      stdio: "inherit",
      env: { ...process.env, OWNER_APPROVED_REVENUE_COLLECTION_TOKEN: "" }
    });
    execSync("node packages/db/src/_verify-1.1m.mjs", { cwd: ROOT, stdio: "inherit" });
    passes++;
  } catch (e) {
