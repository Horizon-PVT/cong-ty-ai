#!/usr/bin/env node
/**
 * Milestone 1.0W: Email Boss Live Test Auto-Loop
...
async function runMission() {
  try {
    execSync("node scripts/ai-company-run-email-boss-live-test-mission.mjs", {
      cwd: ROOT,
      stdio: "inherit",
      encoding: "utf8",
    });
...
async function main() {
  ensureDirs();
  console.log("[1.0W Auto-Loop] Starting Boss Live Test auto-loop...");
  ...
  const report = {
    ...
    verdict: consecutivePasses >= REQUIRED_STABLE_PASSES
      ? "EMAIL_BOSS_LIVE_TEST_AUTO_LOOP_STABLE"
      : "EMAIL_BOSS_LIVE_TEST_AUTO_LOOP_UNSTABLE",
  };
  const reportPath = path.join(LOG_DIR, "email-boss-live-test-auto-loop-report.json");
