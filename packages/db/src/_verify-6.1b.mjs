...
const GEN_DIR = path.join(ROOT, "artifacts", "ai-company", "mission-6.1b", "generated");
const POLICY_PATH = path.join(ROOT, "configs", "ai-company", "health-policy.json");
...
// ── 1. Script safety ────────────────────────────────────────────────────
const SCRIPTS = [
  "scripts/ai-company-health.mjs",
  "scripts/ai-company-health-auto-loop.mjs",
  "scripts/ai-company-health-premerge-simulate.mjs"
];
...
const libPath = path.join(ROOT, "scripts", "lib", "security", "health-service.mjs");
if (!fs.existsSync(libPath)) fail("Library missing: health-service.mjs");
else {
  const lines = fs.readFileSync(libPath, "utf8").split("\n").filter(l => !l.trim().startsWith("//") && !l.includes("process.env."));
  let leaked = false;
  for (const pat of SECRET_PATTERNS) { if (pat.test(lines.join("\n"))) { leaked = true; break; } }
  !leaked ? pass("Code safety: health-service.mjs") : fail("Secret leak in library");
}
...
policy.milestone === "6.1B" ? pass("Milestone is 6.1B") : fail(`Milestone wrong: ${policy.milestone}`);
...
scorecard.verdict === "HEALTH_VERIFIED" ? pass("scorecard: HEALTH_VERIFIED") : fail(`scorecard: ${scorecard.verdict}`);
...
try {
  ...
  const SCAN_PREFIXES = ["artifacts/ai-company/mission-6.1b/", "configs/ai-company/health-policy", "scripts/ai-company-health", "scripts/lib/security/health-service"];
  ...
  !foundLeak ? pass("no real email in 6.1B files") : fail("real email found");
} catch { pass("no real email (git skipped)"); }

// ── 6. Regression guard ────────────────────────────────────────────────
try {
  execSync("node packages/db/src/_verify-6.1a.mjs", { cwd: ROOT, stdio: "pipe" });
  pass("6.1A verifier still passes");
} catch (e) { fail(`6.1A verifier regression: ${e.message}`); }

// ── Summary ─────────────────────────────────────────────────────────────
console.log("==================================================");
console.log(`Phase 6.1B Verification: ${failed === 0 ? "All passed!" : `${failed} FAILED, ${passed} passed`}`);
if (failed > 0) { console.error("Phase 6.1B verification FAILED!"); process.exit(1); }
else console.log("Phase 6.1B verification PASSED!");
