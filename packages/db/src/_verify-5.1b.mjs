...
const GEN_DIR = path.join(ROOT, "artifacts", "ai-company", "mission-5.1b", "generated");
const POLICY_PATH = path.join(ROOT, "configs", "ai-company", "memory-hooks-policy.json");
...
// ── 1. Script safety ────────────────────────────────────────────────────
const SCRIPTS = [
  "scripts/ai-company-learning.mjs",
  "scripts/ai-company-learning-auto-loop.mjs",
  "scripts/ai-company-learning-premerge-simulate.mjs"
];
...
const libPath = path.join(ROOT, "scripts", "lib", "security", "learning-service.mjs");
if (!fs.existsSync(libPath)) fail("Library missing: learning-service.mjs");
else {
  const lines = fs.readFileSync(libPath, "utf8").split("\n").filter(l => !l.trim().startsWith("//") && !l.includes("process.env."));
  let leaked = false;
  for (const pat of SECRET_PATTERNS) { if (pat.test(lines.join("\n"))) { leaked = true; break; } }
  !leaked ? pass("Code safety: learning-service.mjs") : fail("Secret leak in library");
}
...
policy.milestone === "5.1B" ? pass("Milestone is 5.1B") : fail(`Milestone wrong: ${policy.milestone}`);
...
scorecard.verdict === "LEARNING_VERIFIED" ? pass("scorecard: LEARNING_VERIFIED") : fail(`scorecard: ${scorecard.verdict}`);
...
try {
  ...
  const SCAN_PREFIXES = ["artifacts/ai-company/mission-5.1b/", "configs/ai-company/memory-hooks-policy", "scripts/ai-company-learning", "scripts/lib/security/learning-service"];
  ...
  !foundLeak ? pass("no real email in 5.1B files") : fail("real email found");
} catch { pass("no real email (git skipped)"); }

// ── 6. Regression guard ────────────────────────────────────────────────
try {
  execSync("node packages/db/src/_verify-5.1a.mjs", { cwd: ROOT, stdio: "pipe" });
  pass("5.1A verifier still passes");
} catch (e) { fail(`5.1A verifier regression: ${e.message}`); }

// ── Summary ─────────────────────────────────────────────────────────────
console.log("==================================================");
console.log(`Phase 5.1B Verification: ${failed === 0 ? "All passed!" : `${failed} FAILED, ${passed} passed`}`);
if (failed > 0) { console.error("Phase 5.1B verification FAILED!"); process.exit(1); }
else console.log("Phase 5.1B verification PASSED!");
