...
const GEN_DIR = path.join(ROOT, "artifacts", "ai-company", "mission-5.1a", "generated");
const POLICY_PATH = path.join(ROOT, "configs", "ai-company", "memory-policy.json");
...
// ── 1. Script safety ────────────────────────────────────────────────────
const SCRIPTS = [
  "scripts/ai-company-memory.mjs",
  "scripts/ai-company-memory-auto-loop.mjs",
  "scripts/ai-company-memory-premerge-simulate.mjs"
];
...
const libPath = path.join(ROOT, "scripts", "lib", "security", "memory-service.mjs");
if (!fs.existsSync(libPath)) fail("Library missing: memory-service.mjs");
else {
  const lines = fs.readFileSync(libPath, "utf8").split("\n").filter(l => !l.trim().startsWith("//") && !l.includes("process.env."));
  let leaked = false;
  for (const pat of SECRET_PATTERNS) { if (pat.test(lines.join("\n"))) { leaked = true; break; } }
  !leaked ? pass("Code safety: memory-service.mjs") : fail("Secret leak in library");
}
...
policy.milestone === "5.1A" ? pass("Milestone is 5.1A") : fail(`Milestone wrong: ${policy.milestone}`);
...
scorecard.verdict === "MEMORY_VERIFIED" ? pass("scorecard: MEMORY_VERIFIED") : fail(`scorecard: ${scorecard.verdict}`);
...
try {
  ...
  const SCAN_PREFIXES = ["artifacts/ai-company/mission-5.1a/", "configs/ai-company/memory-policy", "scripts/ai-company-memory", "scripts/lib/security/memory-service"];
  ...
  !foundLeak ? pass("no real email in 5.1A files") : fail("real email found");
} catch { pass("no real email (git skipped)"); }

// ── 6. Regression guard ────────────────────────────────────────────────
try {
  execSync("node packages/db/src/_verify-4.1b.mjs", { cwd: ROOT, stdio: "pipe" });
  pass("4.1B verifier still passes");
} catch (e) { fail(`4.1B verifier regression: ${e.message}`); }

// ── Summary ─────────────────────────────────────────────────────────────
console.log("==================================================");
console.log(`Phase 5.1A Verification: ${failed === 0 ? "All passed!" : `${failed} FAILED, ${passed} passed`}`);
if (failed > 0) { console.error("Phase 5.1A verification FAILED!"); process.exit(1); }
else console.log("Phase 5.1A verification PASSED!");
