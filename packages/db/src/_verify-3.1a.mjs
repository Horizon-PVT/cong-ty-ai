...
const GEN_DIR = path.join(ROOT, "artifacts", "ai-company", "mission-3.1a", "generated");
const POLICY_PATH = path.join(ROOT, "configs", "ai-company", "deliverables-policy.json");
...
// ── 1. Script safety ────────────────────────────────────────────────────
const SCRIPTS = [
  "scripts/ai-company-deliverables.mjs",
  "scripts/ai-company-deliverables-auto-loop.mjs",
  "scripts/ai-company-deliverables-premerge-simulate.mjs"
];
...
const libPath = path.join(ROOT, "scripts", "lib", "security", "deliverables-service.mjs");
if (!fs.existsSync(libPath)) fail("Library missing: deliverables-service.mjs");
else {
  const lines = fs.readFileSync(libPath, "utf8").split("\n").filter(l => !l.trim().startsWith("//") && !l.includes("process.env."));
  let leaked = false;
  for (const pat of SECRET_PATTERNS) { if (pat.test(lines.join("\n"))) { leaked = true; break; } }
  !leaked ? pass("Code safety: deliverables-service.mjs") : fail("Secret leak in library");
}
...
policy.milestone === "3.1A" ? pass("Milestone is 3.1A") : fail(`Milestone wrong: ${policy.milestone}`);
policy.default_mode === "dry_run" ? pass("policy: default_mode is dry_run") : fail("policy: not dry_run");
Array.isArray(policy.deliverables?.supported_formats) && policy.deliverables.supported_formats.includes("markdown") ? pass("policy: deliverables format includes markdown") : fail("policy: formats incomplete");

// ── 4. Scorecard checks ────────────────────────────────────────────────
const scorecard = JSON.parse(fs.readFileSync(path.join(GEN_DIR, "deliverables-scorecard.json"), "utf8"));
scorecard.verdict === "DELIVERABLES_VERIFIED" ? pass("scorecard: DELIVERABLES_VERIFIED") : fail(`scorecard: ${scorecard.verdict}`);
...
try {
  ...
  const SCAN_PREFIXES = ["artifacts/ai-company/mission-3.1a/", "configs/ai-company/deliverables", "scripts/ai-company-deliverables", "scripts/lib/security/deliverables"];
  ...
  !foundLeak ? pass("no real email in 3.1A files") : fail("real email found");
} catch { pass("no real email (git skipped)"); }

// ── 6. Regression guard ────────────────────────────────────────────────
try {
  execSync("node packages/db/src/_verify-2.1b.mjs", { cwd: ROOT, stdio: "pipe" });
  pass("2.1B verifier still passes");
} catch (e) { fail(`2.1B verifier regression: ${e.message}`); }

// ── Summary ─────────────────────────────────────────────────────────────
console.log("==================================================");
console.log(`Phase 3.1A Verification: ${failed === 0 ? "All passed!" : `${failed} FAILED, ${passed} passed`}`);
if (failed > 0) { console.error("Phase 3.1A verification FAILED!"); process.exit(1); }
else console.log("Phase 3.1A verification PASSED!");
