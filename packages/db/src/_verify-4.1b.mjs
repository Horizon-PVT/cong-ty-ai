...
const GEN_DIR = path.join(ROOT, "artifacts", "ai-company", "mission-4.1b", "generated");
const POLICY_PATH = path.join(ROOT, "configs", "ai-company", "auto-runtime-policy.json");
...
// ── 1. Script safety ────────────────────────────────────────────────────
const SCRIPTS = [
  "scripts/ai-company-auto-runtime.mjs",
  "scripts/ai-company-auto-runtime-auto-loop.mjs",
  "scripts/ai-company-auto-runtime-premerge-simulate.mjs"
];
...
const libPath = path.join(ROOT, "scripts", "lib", "security", "auto-runtime-service.mjs");
if (!fs.existsSync(libPath)) fail("Library missing: auto-runtime-service.mjs");
else {
  const lines = fs.readFileSync(libPath, "utf8").split("\n").filter(l => !l.trim().startsWith("//") && !l.includes("process.env."));
  let leaked = false;
  for (const pat of SECRET_PATTERNS) { if (pat.test(lines.join("\n"))) { leaked = true; break; } }
  !leaked ? pass("Code safety: auto-runtime-service.mjs") : fail("Secret leak in library");
}
...
policy.milestone === "4.1B" ? pass("Milestone is 4.1B") : fail(`Milestone wrong: ${policy.milestone}`);
policy.default_mode === "dry_run" ? pass("policy: default_mode is dry_run") : fail("policy: not dry_run");
policy.runtime?.max_execution_steps === 20 ? pass("policy: max_execution_steps is 20") : fail("policy: wrong limit");

// ── 4. Scorecard checks ────────────────────────────────────────────────
const scorecard = JSON.parse(fs.readFileSync(path.join(GEN_DIR, "deliverables-scorecard.json"), "utf8"));
scorecard.verdict === "AUTO_RUNTIME_VERIFIED" ? pass("scorecard: AUTO_RUNTIME_VERIFIED") : fail(`scorecard: ${scorecard.verdict}`);
...
try {
  ...
  const SCAN_PREFIXES = ["artifacts/ai-company/mission-4.1b/", "configs/ai-company/auto-runtime", "scripts/ai-company-auto-runtime", "scripts/lib/security/auto-runtime"];
  ...
  !foundLeak ? pass("no real email in 4.1B files") : fail("real email found");
} catch { pass("no real email (git skipped)"); }

// ── 6. Regression guard ────────────────────────────────────────────────
try {
  execSync("node packages/db/src/_verify-4.1a.mjs", { cwd: ROOT, stdio: "pipe" });
  pass("4.1A verifier still passes");
} catch (e) { fail(`4.1A verifier regression: ${e.message}`); }

// ── Summary ─────────────────────────────────────────────────────────────
console.log("==================================================");
console.log(`Phase 4.1B Verification: ${failed === 0 ? "All passed!" : `${failed} FAILED, ${passed} passed`}`);
if (failed > 0) { console.error("Phase 4.1B verification FAILED!"); process.exit(1); }
else console.log("Phase 4.1B verification PASSED!");
