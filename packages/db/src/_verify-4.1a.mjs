...
const GEN_DIR = path.join(ROOT, "artifacts", "ai-company", "mission-4.1a", "generated");
const POLICY_PATH = path.join(ROOT, "configs", "ai-company", "runtime-safety-policy.json");
...
// ── 1. Script safety ────────────────────────────────────────────────────
const SCRIPTS = [
  "scripts/ai-company-runtime-safety.mjs",
  "scripts/ai-company-runtime-safety-auto-loop.mjs",
  "scripts/ai-company-runtime-safety-premerge-simulate.mjs"
];
...
const libPath = path.join(ROOT, "scripts", "lib", "security", "runtime-safety-service.mjs");
if (!fs.existsSync(libPath)) fail("Library missing: runtime-safety-service.mjs");
else {
  const lines = fs.readFileSync(libPath, "utf8").split("\n").filter(l => !l.trim().startsWith("//") && !l.includes("process.env."));
  let leaked = false;
  for (const pat of SECRET_PATTERNS) { if (pat.test(lines.join("\n"))) { leaked = true; break; } }
  !leaked ? pass("Code safety: runtime-safety-service.mjs") : fail("Secret leak in library");
}
...
policy.milestone === "4.1A" ? pass("Milestone is 4.1A") : fail(`Milestone wrong: ${policy.milestone}`);
policy.default_mode === "dry_run" ? pass("policy: default_mode is dry_run") : fail("policy: not dry_run");
policy.safety?.max_consecutive_failures === 3 ? pass("policy: max_consecutive_failures is 3") : fail("policy: wrong limit");

// ── 4. Scorecard checks ────────────────────────────────────────────────
const scorecard = JSON.parse(fs.readFileSync(path.join(GEN_DIR, "deliverables-scorecard.json"), "utf8"));
scorecard.verdict === "SAFETY_VERIFIED" ? pass("scorecard: SAFETY_VERIFIED") : fail(`scorecard: ${scorecard.verdict}`);
...
try {
  ...
  const SCAN_PREFIXES = ["artifacts/ai-company/mission-4.1a/", "configs/ai-company/runtime-safety", "scripts/ai-company-runtime-safety", "scripts/lib/security/runtime-safety"];
  ...
  !foundLeak ? pass("no real email in 4.1A files") : fail("real email found");
} catch { pass("no real email (git skipped)"); }

// ── 6. Regression guard ────────────────────────────────────────────────
try {
  execSync("node packages/db/src/_verify-3.1b.mjs", { cwd: ROOT, stdio: "pipe" });
  pass("3.1B verifier still passes");
} catch (e) { fail(`3.1B verifier regression: ${e.message}`); }

// ── Summary ─────────────────────────────────────────────────────────────
console.log("==================================================");
console.log(`Phase 4.1A Verification: ${failed === 0 ? "All passed!" : `${failed} FAILED, ${passed} passed`}`);
if (failed > 0) { console.error("Phase 4.1A verification FAILED!"); process.exit(1); }
else console.log("Phase 4.1A verification PASSED!");
