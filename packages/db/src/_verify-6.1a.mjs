...
const GEN_DIR = path.join(ROOT, "artifacts", "ai-company", "mission-6.1a", "generated");
const POLICY_PATH = path.join(ROOT, "configs", "ai-company", "teamwork-policy.json");
...
// ── 1. Script safety ────────────────────────────────────────────────────
const SCRIPTS = [
  "scripts/ai-company-teamwork.mjs",
  "scripts/ai-company-teamwork-auto-loop.mjs",
  "scripts/ai-company-teamwork-premerge-simulate.mjs"
];
...
const libPath = path.join(ROOT, "scripts", "lib", "security", "teamwork-service.mjs");
if (!fs.existsSync(libPath)) fail("Library missing: teamwork-service.mjs");
else {
  const lines = fs.readFileSync(libPath, "utf8").split("\n").filter(l => !l.trim().startsWith("//") && !l.includes("process.env."));
  let leaked = false;
  for (const pat of SECRET_PATTERNS) { if (pat.test(lines.join("\n"))) { leaked = true; break; } }
  !leaked ? pass("Code safety: teamwork-service.mjs") : fail("Secret leak in library");
}
...
policy.milestone === "6.1A" ? pass("Milestone is 6.1A") : fail(`Milestone wrong: ${policy.milestone}`);
...
scorecard.verdict === "TEAMWORK_VERIFIED" ? pass("scorecard: TEAMWORK_VERIFIED") : fail(`scorecard: ${scorecard.verdict}`);
...
try {
  ...
  const SCAN_PREFIXES = ["artifacts/ai-company/mission-6.1a/", "configs/ai-company/teamwork-policy", "scripts/ai-company-teamwork", "scripts/lib/security/teamwork-service"];
  ...
  !foundLeak ? pass("no real email in 6.1A files") : fail("real email found");
} catch { pass("no real email (git skipped)"); }

// ── 6. Regression guard ────────────────────────────────────────────────
try {
  execSync("node packages/db/src/_verify-5.1b.mjs", { cwd: ROOT, stdio: "pipe" });
  pass("5.1B verifier still passes");
} catch (e) { fail(`5.1B verifier regression: ${e.message}`); }

// ── Summary ─────────────────────────────────────────────────────────────
console.log("==================================================");
console.log(`Phase 6.1A Verification: ${failed === 0 ? "All passed!" : `${failed} FAILED, ${passed} passed`}`);
if (failed > 0) { console.error("Phase 6.1A verification FAILED!"); process.exit(1); }
else console.log("Phase 6.1A verification PASSED!");
